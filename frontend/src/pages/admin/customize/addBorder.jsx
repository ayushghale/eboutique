...
export default function AddBorder() {
  ...
  const [editBordeID, setEditBorderID] = useState(id);
  ...
  const checkEmpty = () => {
    ...
    if (!editBordeID) {
      if (!image) {
        toast.error("Please select an image qqq");
        return false;
      }
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await adminApi.post("admin/border/add", formData);

      setIsOpen(false);
      // Check if the request was successful (status code 2xx)
      if (response) {
        toast("border added successfully");
        clearForm();
        // Add any additional logic or redirect as needed
      } else {
        toast.error("Failed to add border:", response);
      }
    } catch (error) {
  ...
  const getBorder = async () => {
    try {
      const response = await adminApi.get(`admin/border/${editBordeID}`);
      ...
      if (response) {
        ...
        toast.error("Failed to fetch border");
      }
    } catch (error) {
      toast.error("Error fetching border:", error.message);
    }
  };
  ...
  const updateBorder = async () => {
    ...
    try {
      const response = await adminApi.post(
        `admin/border/update/${editBordeID}`,
        formData
      );
      setIsOpen(false);
      if (response.success === true) {
        toast("Border updated successfully");
        clearForm();
        navigate("/admin/border/display");
      } else {
        toast.error("Failed to update Border:", response);
      }
    } catch (error) {
      toast.error("Error sending request:", error.message);
      setIsOpen(false);
            ...
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <nav>
                <ol className="flex items-center gap-2">
                  ...
                  <li className="font-medium text-primary">Border</li>/
                  ...
                  <div className=" p-7  ">
                    <div className="flex justify-between gap-4.5">
                      <button
                        ...
                        type="submit"
                        onClick={editBordeID ? updateBorder : handleSubmit}
                      >
                        Save
                      </button>
                      <button
